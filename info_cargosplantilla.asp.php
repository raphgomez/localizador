...
<title>info_cargosplantilla.asp.php</title>
<meta charset="utf-8">
...
<div style="position: fixed; top: 0pt;" class="w3-card-2 topnav" id="topnav">
  <div style="overflow:auto;">
    <div class="w3-bar w3-left" style="width:55%;overflow:hidden;height:44px">
      ...
      <a href="info_objetivos.asp.php" class="w3-hide-small" title="Noticias">❮</a><img src="img/entrenamiento3.png" width="35" height="32"/> CONTROL DE CARGOS EN PLANTILLA</a>
	  </div>
	<div class="w3-bar w3-right" style="width:45%;overflow:hidden;height:44px;">
	  <a href="Insertar_Cargo_Plantilla.php"title="Insertar Cargos a la Plantilla"><img src="img/entrenamiento3.png" width="37" height="30"/></a>
	  <?php echo "" . $_SESSION['valid_user']; ?></div>
  </div>
</div>
<hr>
...
Highcharts.chart('container', {
    ...
    title: {
        text: 'Total de Cargos en Plantilla'
    },
    subtitle: {
        text: 'Fuente: Listado de Cargos de la Plantilla'
    },
    plotOptions: {
        column: {
            depth: 35
    ...
    series: [{
        ...
        data: [<?php 
				...
				`personal_plantilla_anec`.`Area`,
				Count(`personal_plantilla_anec`.`Area`),
				personal_plantilla_anec.id_plantillacargos,
				personal_plantilla_anec.`date`,
				personal_plantilla_anec.nombre_cargo,
				personal_plantilla_anec.Sexo,
				personal_plantilla_anec.Nivel_Escolaridad,
				personal_plantilla_anec.Especialidad,
				personal_plantilla_anec.Categoria_ocupacional,
				personal_plantilla_anec.Grado_Cientifico,
				personal_plantilla_anec.Observaciones,
				personal_plantilla_anec.Estado,
				areas.Codigo,
				areas.Area
				FROM
				personal_plantilla_anec
				Inner Join areas ON areas.Area = personal_plantilla_anec.Area
				GROUP BY `personal_plantilla_anec`.`Area`");
		...
		<script type="text/javascript">
			...
			$(document).ready(function() {
				chart = new Highcharts.Chart({
					...
					title: {
						text: 'Total de Cargos de Plantilla Cubiertos/por Ocupar'
					},
					tooltip: {
						formatter: function() {
							return '<b>'+ this.point.name +'</b>: '+ this.y +' Persona(s)';
						}
					},
					plotOptions: {
						pie: {
				    ...
				    series: [{
						...
						$r = mysql_query("SELECT
						`personal_plantilla_anec`.`Estado`,
						Count(`personal_plantilla_anec`.`Estado`),
						personal_plantilla_anec.id_plantillacargos,
						personal_plantilla_anec.`date`,
						personal_plantilla_anec.nombre_cargo,
						personal_plantilla_anec.Sexo,
						...
						FROM
						personal_plantilla_anec
						GROUP BY `personal_plantilla_anec`.`Estado`");
...
function filtrar(){
  document.getElementById("result").innerHTML = "<iframe src='filtrar_cargosplantilla.asp.php?comentario="+document.getElementById("comentario").value+"' frameborder=700 width=1230 height=750 scroll=100>";
}
</script>
  <div align="center">
  <body onLoad="filtrar();">
