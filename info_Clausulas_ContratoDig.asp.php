...
<title>info_Clausulas_ContratoDig.asp.php</title>
<meta charset="utf-8">
...
<div style="position: fixed; top: 0pt;" class="w3-card-2 topnav" id="topnav">
  <div style="overflow:auto;">
    <div class="w3-bar w3-left" style="width:55%;overflow:hidden;height:44px">
      <a href="index2.php" title="Menu">☰</a>❮<a><img src="img/documentos.PNG" width="37" height="36"/> CONTROL DE CLAUSULAS DEL CONTRATO</a>
	  </div>
	<div class="w3-bar w3-right" style="width:45%;overflow:hidden;height:44px;">
	  <a href="Inser_AlcanceSistema.php"title="Insertar Alcance del Sistema"> <img src="img/documentos.PNG" width="37" height="36"/>+</a>
	  <a href="Inser_ClausulasContrato.php" title="Insertar Clausulas Generales"><img src="img/59dd83353b8e6ade996ff4cdd8f99ad0.png" width="37" height="36"/></a>
	  <a href="Formulario_Insertar_ClausContrato.php" title="Insertar Elementos de las Clausulas"><img src="img/result-right.png" width="37" height="36"/></a>
	  <script language="JavaScript">
		...
		</script><a href="javascript:AbrirCentrado('Actualizar_Estado_Requisitos.php','Pagina','630','391');"><img src="img/arrows.png" title="Actualizar Estado del Requisito" width="37" height="36"/></a>
...
Highcharts.chart('container', {
    ...
    title: {
        text: 'Total de Requisitos Declarados'
    },
    subtitle: {
        text: 'Fuente: Listado de Requisitos Declarados'
    },
    plotOptions: {
        column: {
            depth: 35
    ...
    series: [{
        ...
        data: [<?php 
				...
				`contenidos_clausulas`.`Estado_claus`,
				Count(`contenidos_clausulas`.`Estado_claus`),
				`contenidos_clausulas`.`id_contenidosclausulas`,
				`contenidos_clausulas`.`id_clausulacontrato`,
				`contenidos_clausulas`.`contenido_clausula`,
				`contenidos_clausulas`.`requisito_productoServicio`,
				`contenidos_clausulas`.`id_tiposrequisitos`,
				`contenidos_clausulas`.`Especific_requisito`,
				`contenidos_clausulas`.`Especific_requisito1`,
				`contenidos_clausulas`.`Estado_claus`,
				`contratos`.`id_contrato`,
				`contratos`.`comentario`
				FROM
				`contenidos_clausulas`
				Inner Join `contratos` ON `contratos`.`id_contrato` = `contenidos_clausulas`.`id_contrato`
				GROUP BY Estado_claus");
				$a = mysql_fetch_array($r);
				for($b=0;$b<mysql_num_rows($r);$b++)
				   {
					 mysql_data_seek($r,$b);
					 $a = mysql_fetch_array($r);
				?>['<?php echo ''.$a["Estado_claus"].'';?>',
				<?php echo ''.$a[COUNT("Estado_claus")].'';?>],<?php 
				   }?>]
    }]
});
		</script>
		...
		<script type="text/javascript">
			...
			$(document).ready(function() {
				chart = new Highcharts.Chart({
					...
					title: {
						text: 'Total No Conformidades por Estado'
					},
					tooltip: {
						formatter: function() {
							return '<b>'+ this.point.name +'</b>: '+ this.y +' No Conformidad(es)';
						}
					},
					plotOptions: {
						pie: {
...
<td width="83"><html>
  ...
  <script language="JavaScript">
function filtrar(){
  document.getElementById("result").innerHTML = "<iframe src='filtrar_clausulas.asp.php?comentario="+document.getElementById("comentario").value+"' frameborder=1270 width=1270 height=750 scroll=100>";
}
</script>
  <div align="center">
  <body onLoad="filtrar();">
...
FROM
contenidos_clausulas
ORDER BY id_contenidosclausulas DESC");
$a = mysql_fetch_array($r);
?>	
<?php 
...
FROM
contenidos_clausulas
ORDER BY id_contenidosclausulas DESC";  // sentencia sql 
$r = mysql_query($sql); 
...
//echo ' ' . $numero.' registros';  // imprimos en pantalla el n�mero generado 
					...
					if ($numero >=0)
						  {
						 ?><?php echo'<script language="javascript">alert("Hola '.$_SESSION['valid_user'].', usted ya ha establecido en QX3M-7.Mangrisk! '.$numero.' requisitos del Cliente en su Sistema de Gestion. Por favor, dirijase al modulo Satisfaccion del Cliente.");</script>';	

						  }
					else 
						  {
							echo '<script language="javascript">alert("! Hola '.$_SESSION['valid_user'].', usted NO ha establecido en QX3M-7.Mangrisk! los requisitos del Cliente en su Sistema de Gestion. Por favor, determinelos.");</script>';					
							}						
?>
<?php
}
