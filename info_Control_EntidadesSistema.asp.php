...
<title>info_Control_EntidadesSistema.asp.php</title>
<meta charset="utf-8">
...
<div style="position: fixed; top: 0pt;" class="w3-card-2 topnav" id="topnav">
  <div style="overflow:auto;">
    <div class="w3-bar w3-left" style="width:55%;overflow:hidden;height:44px">
      <a href="Dashboards.php" title="Menu">☰</a><a href="procesoprimero.php" >❮<a>CONTROL DE ENTIDADES CON SISTEMA</a>
	  </div>
	<div class="w3-bar w3-right" style="width:45%;overflow:hidden;height:44px;">
	  <a href="Formulario_Insertar_Entidad.php" title="Insertar entidades"> <img src="img/documentos.PNG" width="37" height="36"/>+</a>
	  <a href=" " title=" "><img src="img/nc.png" width="37" height="36"/></a>
	  <a href="Actualizar_Estado_Entidad.php" title="Actualizar estado de la entidad"><img src="img/ARROWS.png" width="37" height="36"/></a>
	  <a href="" title="Descargar data"><img src="img/Dark_Off.png" width="37" height="34"/></a>
...
Highcharts.chart('container', {
    ...
    title: {
        text: 'Total de Entidades con Sistema'
    },
    subtitle: {
        text: 'Fuente: Listado de Entidades Conectadas al Sistema'
    },
    plotOptions: {
        column: {
            depth: 35
    ...
    series: [{
        ...
        data: [<?php 
				...
				`documentos_dig`.`Area`,
				Count(documentos_dig.Area),
				`documentos_dig`.`id`,
				`documentos_dig`.`valid_user`,
				`documentos_dig`.`date`,
				`documentos_dig`.`Sist_Gestion`,
				`documentos_dig`.`Area`,
				`documentos_dig`.`Codigo`,
				`documentos_dig`.`Version`,
				`documentos_dig`.`Copia`,
				`documentos_dig`.`Estado_doc`,
				`documentos_dig`.`Observaciones`,
				`documentos_dig`.`ruta_fichero`,
				`documentos_dig`.`comentario`
				FROM
				`documentos_dig`
				GROUP BY `documentos_dig`.`Area`");
		...
		<script type="text/javascript">
			...
			$(document).ready(function() {
				chart = new Highcharts.Chart({
					...
					title: {
						text: 'Total Documentos por Estado y NC de Diagnostico'
					},
					tooltip: {
						formatter: function() {
							return '<b>'+ this.point.name +'</b>: '+ this.y +' Documento (s)';
						}
					},
					plotOptions: {
						pie: {
				    ...
				    series: [{
						...
						data: [<?php 
										...
										$r = mysql_query("SELECT
										documentos_dig.Estado_doc,
										Count(documentos_dig.Estado_doc),
										documentos_dig.id
										FROM
										documentos_dig
										GROUP BY documentos_dig.Estado_doc");
													  ...
													  for($b=0;$b<mysql_num_rows($r);$b++)
													   {
														 ...
														 $a = mysql_fetch_array($r);
															...
															?>['<?php echo ''.$a["Estado_doc"].'';?>',
															    <?php echo ''.$a[COUNT("Estado_doc")].'';?>],<?php 
...
function filtrar(){
  document.getElementById("result").innerHTML = "<iframe src='filtrar_entidadesistema.asp.php?comentario="+document.getElementById("comentario").value+"' frameborder=1270 width=1270 height=850 scroll=100>";
}
</script>
  <div align="center">
  <body onLoad="filtrar();">
...
<div align="right"><div id="siteInfo"> Buscar:<input name="text" type="text" id="comentario" onKeyUp="filtrar();">  
</div>
<div id="result"></div>
</html></td>
    </tr>
...
<div style="position: fixed; bottom: 0px;" class="w3-card-2 w3-slim topnav" id="topnav">
  <div style="overflow:auto;">
    <ul class="w3-navbar w3-left" style="width:25%;overflow:hidden;height:4px">
      ...
      </div><marquee><p align="center"> <a href="www.facebook.com/consultor">Contactenos:</a> www.facebook.com/consultor/</div> </a> 
  </marquee></div>
<hr>        
<br><br>
</div>
