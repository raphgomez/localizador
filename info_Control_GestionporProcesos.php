...
<title>info_Control_GestionporProcesos.php</title>
<meta charset="utf-8">
...
<div style="position: fixed; top: 0pt;" class="w3-card-2 topnav" id="topnav">
  <div style="overflow:auto;">
    <div class="w3-bar w3-left" style="width:55%;overflow:hidden;height:44px">
      <a href="Dashboards.php" title="Menu">☰</a><a href="procesoprimero.php" >❮<a><img src="img/4149680.png" width="37" height="36"/> GESTION POR PROCESOS</a>
	  </div>
	<div class="w3-bar w3-right" style="width:45%;overflow:hidden;height:44px;">
	  <a href="Formulario_Insertar_GestionProcesos.php"title="Insertar interacciones"> <img src="img/4149680.png" width="37" height="36"/>+</a>
	  <script language="JavaScript">
		function AbrirCentrado(Url,NombreVentana,width,height,extras) {
		var largo = width;
		var altura = height;
		var adicionales= extras;
		var top = (screen.height-altura)/8;
		...
		</script><a href="javascript:AbrirCentrado('Formulario_SeleccionInteraccion.php','Pagina','830','291');"><img src="img/arrows.png" title="Actualizar interacciones" width="37" height="36"/></a>
...
Highcharts.chart('container', {
    ...
    title: {
        text: 'Total de Interacciones de Procesos'
    },
    subtitle: {
        text: 'Fuente: Listado de Interacciones de los Procesos'
    },
    plotOptions: {
        column: {
            depth: 35
    ...
    series: [{
        ...
        data: [<?php 
				...
				`procesos`.`Proceso`,
				Count(`procesos`.`Proceso`),
				`gestion_procesos`.`id_gestpro`,
				`gestion_procesos`.`Entrada`,
				`gestion_procesos`.`id_personalplantilla`,
				`gestion_procesos`.`CriterioAcep`,
				`gestion_procesos`.`Proceso_Proveedor`,
				`gestion_procesos`.`Proceso_Cliente`,
				`gestion_procesos`.`CriterioAcep1`,
				`gestion_procesos`.`Salida`,
				`procesos`.`id_proceso`,
				`procesos`.`Proceso`
				FROM
				`gestion_procesos`
				Inner Join `procesos` ON `procesos`.`id_proceso` = `gestion_procesos`.`id_proceso`
				GROUP BY `procesos`.`Proceso`");
		...
		<script type="text/javascript">
			...
			$(document).ready(function() {
				chart = new Highcharts.Chart({
					...
					title: {
						text: 'Total interacciones por Procesos'
					},
					tooltip: {
						formatter: function() {
							return '<b>'+ this.point.name +'</b>: '+ this.y +' interacciones (s)';
						}
					},
					plotOptions: {
						pie: {
...
<td width="83"><html>
  ...
  <script language="JavaScript">
function filtrar(){
  document.getElementById("result").innerHTML = "<iframe src='filtrar_interaccionesProcesos.asp.php?comentario="+document.getElementById("comentario").value+"' frameborder=1270 width=1270 height=450 scroll=100>";
}
</script>
  <div align="center">
  <body onLoad="filtrar();">
