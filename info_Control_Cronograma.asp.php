...
if(isset($_SESSION["valid_user"]))
	{
	   ...
	   $r = mysql_query("
		...
		permissions.id_script,
		permissions.nombre_script
		FROM
		role_perm
		Inner Join roles ON roles.role_id = role_perm.role_id
		Inner Join permissions ON permissions.perm_id = role_perm.perm_id
	   WHERE roles.role_name='$usn' AND permissions.perm_id='74'");
	   $a = mysql_fetch_array($r);
	   for($b=0;$b<mysql_num_rows($r);$b++)
			   {
				 mysql_data_seek($r,$b);
				 $a = mysql_fetch_array($r);
				 if($a["role_name"]=$usn && $a["perm_desc"]="AddActivCronograma")
					{
					?>
					<!DOCTYPE html>
					...
					<div style="position: fixed; top: 0pt;" class="w3-card-2 topnav" id="topnav">
					  <div style="overflow:auto;">
						<div class="w3-bar w3-left" style="width:55%;overflow:hidden;height:44px">
						  <a href="Dashboards.php" title="Menu">☰</a><a href="procesoprimero.php" >❮<a><img src="img/documentos.PNG" width="37" height="36"/>CONTROL DE CRONOGRAMA DE IMPLEMENTACION</a>
						  </div>
					...
					function filtrar(){
					  document.getElementById("result").innerHTML = "<iframe src='filtrar_cronograma.asp.php?comentario="+document.getElementById("comentario").value+"' frameborder=1270 width=1270 height=650 scroll=100>";
					}
					</script>
					  <div align="center">
					  <body onLoad="filtrar();">
					...
					<div style="position: fixed; bottom: 0px;" class="w3-card-2 w3-slim topnav" id="topnav">
						<ul class="w3-navbar w3-left" style="width:25%;overflow:hidden;height:4px"></ul>
